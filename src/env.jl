...
immutable Env
    atm::Atm
    calibrated_airspeed::SIVelocity{Float64}
    equivalent_airspeed::SIVelocity{Float64}
    true_airspeed::SIVelocity{Float64}
    mach_number::Float64
    q::SIPressure{Float64}
end
...
function calc_mach(calibrated_airspeed::SIVelocity{Float64},pressure_ratio::Float64)
    sqrt(5*((((0.2*(calibrated_airspeed/SeaLevelSpeedOfSound)^2+1)^3.5 - 1)/pressure_ratio+1)^(1/3.5) - 1))
end

function Env(airspeed::Airspeed{kcas},altitude::SILength{Float64},delta_temperature::SITemperature{Float64} = 0.0K)
...
function Env(airspeed::Airspeed{mach},altitude::SILength{Float64},delta_temperature::SITemperature{Float64} = 0.0K)
    atm = Atm(altitude,delta_temperature)
...
function Env(airspeed::Airspeed{keas},altitude::SILength{Float64},delta_temperature::SITemperature{Float64} = 0.0K)
    atm = Atm(altitude,delta_temperature)
